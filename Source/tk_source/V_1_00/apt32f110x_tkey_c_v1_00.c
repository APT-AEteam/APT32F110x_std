...
volatile U16_T tk_press_time_cnt;
/****************************************************
...
//ReturnValue:NONE
/*************************************************************/ 
void coret_tk_intial(void)
{
	CORET_DeInit();
	SYSCON_General_CMD(ENABLE,ENDIS_SYSTICK);
	CK802->CORET_RVR=TK_BaseCnt;						//CORETCLK=sysclock/8=48M/8=6Mhz  e.g:10ms=(CORET_RVR+1)*(8/48M);CORET_RVR=60000-1=59999
	CORET_reload();																
	CORET_CLKSOURCE_EX();								
...
void tkey_init(void)
{
	...
	*(uint32_t *)0x40011240 = TK_Scan_Freq0;
	...
	tkey_io_enable();
	tkey_pwrsrc_config();
	TKEY->TCH_CCR = TK_CLK_EN | TK_DCHCKSEL_REFCLK | TK_PCKDIV(5) | TK_ALDTRIM_DIS |  TK_TRMTARSEL_TRIM | TK_DCHCKDIV_6 | TK_TRIMCONFIG0(10) | TK_TRIMCONFIG1(13) | TK_TRIMCONFIG2(15);
