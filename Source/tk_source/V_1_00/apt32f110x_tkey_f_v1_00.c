...
volatile U8_T tk_error_cnt;
volatile U8_T tk_negtive_build0[32];
volatile U8_T tk_postive_build0[32];
volatile U8_T tk_negtive_build1[32];
volatile U8_T tk_postive_build1[32];
volatile U8_T tk_negtive_build2[32];
volatile U8_T tk_postive_build2[32];
volatile U8_T press_debounce0[32];
volatile U8_T release_debounce0[32];
volatile U8_T press_debounce1[32];
volatile U8_T release_debounce1[32];
volatile U8_T press_debounce2[32];
volatile U8_T release_debounce2[32];
volatile U8_T trigger_cnt;
volatile U8_T scan_step;
/****************************************************
//TK 16bits variable define
*****************************************************/
volatile S16_T offset_data0[32];
volatile S16_T offset_data1[32];
volatile S16_T offset_data2[32];
volatile U16_T sampling_data0[32];
volatile U16_T baseline_data0[32];
volatile U16_T sampling_data1[32];
volatile U16_T baseline_data1[32];
volatile U16_T sampling_data2[32];
volatile U16_T baseline_data2[32];
volatile U16_T offset_data0_abs[32];
volatile U16_T offset_data1_abs[32];
volatile U16_T offset_data2_abs[32];
volatile U16_T offset0_max_tempA;
volatile U16_T offset0_max_tempB;
volatile U16_T offset1_max_tempA;
volatile U16_T offset1_max_tempB;
volatile U16_T offset2_max_tempA;
volatile U16_T offset2_max_tempB;
...
volatile U32_T key_map1;
volatile U32_T key_map2;
volatile U32_T key_map_temp0;
volatile U32_T key_map_temp1;
volatile U32_T tk_press_time_cnt;
/****************************************************
...
void std_tkey_process(void)
{
	...
	TK_error_detect();
	if((Key_Map!=0)&&(TK_Rebuild_Time!=0))
	{ 
		tk_press_time_cnt++;
		if(tk_press_time_cnt>(TK_Rebuild_Time*100))		//cnt base 10ms*100=1s
		{
			base_update_f=1;
			tk_press_time_cnt=0;
		}
...
void TK_Sampling_prog(void)
{
	...
	for (i=0;i<tk_num;i++)
	{
		if(scan_step==1)sampling_data0[(TKEY->TCH_DAT[i]&0xff000000)>>24]=TKEY->TCH_DAT[i]&0X00FFFFFF;
		if(scan_step==2)sampling_data1[(TKEY->TCH_DAT[i]&0xff000000)>>24]=TKEY->TCH_DAT[i]&0X00FFFFFF;
		if(scan_step==0)sampling_data2[(TKEY->TCH_DAT[i]&0xff000000)>>24]=TKEY->TCH_DAT[i]&0X00FFFFFF;
	}
	if(poweron_1st_f<5)
	{
	for (i=0;i<32;i++)
...
void TK_Keymap_prog(void)
{
	...
	for(i=0;i<32;i++)
	{
		...
		if(MultiTimes_Filter>=4)
		{
			if((offset_data1[i]>TK_Triggerlevel[i])&&(offset_data1[i]<(TK_Triggerlevel[i]*MultiTimes_Filter))&&(valid_key_f==0))
		  {
			press_debounce1[i]++;
			release_debounce1[i]=0;
			tk_postive_build1[i]=0;
			tk_negtive_build1[i]=0;
			if(press_debounce1[i]>Press_debounce)
			{
		...
		if((offset_data1[i]>TK_Triggerlevel[i])&&(valid_key_f==0))
		{
			...
			if(press_debounce1[i]>Press_debounce)
			{
				if((Key_mode==0)&&(key_map1==0))			//first single key
				{
				key_map1|=0x01<<i;
				}
				if(Key_mode==1)								//multi key
		...
		if(offset_data1[i]<(TK_Triggerlevel[i]*4/5))
		{
			release_debounce1[i]++;
			press_debounce1[i]=0;
			if(release_debounce1[i]>Release_debounce)
			{
				key_map1&=~(0x01<<i);
				release_debounce1[i]=0;
			}
		...
		if(MultiTimes_Filter>=4)
		{
			if((offset_data2[i]>TK_Triggerlevel[i])&&(offset_data2[i]<(TK_Triggerlevel[i]*MultiTimes_Filter))&&(valid_key_f==0))
		  {
			press_debounce2[i]++;
			release_debounce2[i]=0;
			tk_postive_build2[i]=0;
			tk_negtive_build2[i]=0;
			if(press_debounce2[i]>Press_debounce)
			{
		...
		if((offset_data2[i]>TK_Triggerlevel[i])&&(valid_key_f==0))
		{
			...
			if(press_debounce2[i]>Press_debounce)
			{
				if((Key_mode==0)&&(key_map2==0))			//first single key
				{
				key_map2|=0x01<<i;
				}
				if(Key_mode==1)								//multi key
		...
		if(offset_data2[i]<(TK_Triggerlevel[i]*4/5))
		{
			release_debounce2[i]++;
			press_debounce2[i]=0;
			if(release_debounce2[i]>Release_debounce)
			{
				key_map2&=~(0x01<<i);
				release_debounce2[i]=0;
			}
...
{
	U32_T r_key_map_temp;
	r_key_map_temp=key_map0|key_map1|key_map2;
	if(r_key_map_temp!=0)
	{
		if((key_map0==key_map1)&&(key_map1==key_map2))	//三组频率按键一致
		{
...
{
	U8_T i;
	if((key_map1!=0)&&(trigger_auto_f==0))
	{
		if(key_map_temp1==key_map1)
		{
			return;
		}
		else
		{
			key_map_temp1=key_map1;
			trigger_auto_f=1;
		}
		for(i=0;i<32;i++)
		{
		if(offset_data1_abs[i]>offset_max_data[i])
		{
			offset_max_data[i]=offset_data1_abs[i];
		}
		}
	}
	if((trigger_auto_f==1)&&(key_map1==0))
	{
		for(i=0;i<32;i++)
		{
	...
	if(trigger_auto_f==2)		
	{
		...
		for(i=0;i<32;i++)
		{
			if ((key_map_temp1 & (1<<i))!=0)
			{
				...
				if(TK_Triggerlevel[i]<30)TK_Triggerlevel[i]=30;
				trigger_cnt++;
				if(trigger_cnt==tk_num)
				{
					trigger_cnt=0;
					trigger_auto_f=4;
					key_map_temp1=0;
...
void tkey_init(void)
{
	...
	*(uint32_t *)0x40011240 = TK_Scan_Freq0;
	...
	tk_num=get_key_seq();
	TKEY->TCH_CON0 = TK_HM_EN | TK_MODE_SEQ | TK_SEQLEN(tk_num) | TK_EC_LEVEL | TK_SCTIMLMT_5ms | TK_SCTLIM_EN | TK_INTVTIM_None | TK_PSEL_MODE | TK_INTREN_DIS | TK_STPATDSL_DIS | TK_STBLCNT_16 |TK_CYCSCNT_EN;
	...
	TKEY->TCH_IMCR |= TK_SEQ_DNE|TK_CH_ERR|TK_TIMOVF|TK_REFOVF;
